...
Fixtures/          test fixtures (generated by script, never committed binaries > 100 KB;
                   the two NTFS images are the documented exception — see below)
...
- **The 100 KB fixture rule has exactly two exceptions**, both NTFS images under
  `Tests/PCPluginHostTests/Fixtures/fsimage` (138 KB and 150 KB gzipped). They cannot be
  made smaller: an NTFS volume's own metadata is the floor, and `$UpCase` alone is a
  128 KB table of UTF-16 mappings that barely compresses. Even an 8 MB volume — the
  smallest `mkntfs` will make — lands at 138 KB. The alternative was leaving NTFS
  untested, which is the failure the rest of that plugin's testing goes out of its way to
  avoid. Anything else over 100 KB is still a mistake.
