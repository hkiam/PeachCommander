...
// content_field.swift — let the host's own search look inside a .class (F-351).
//
...
public func ContentGetValue(_ fileName: UnsafeMutablePointer<CChar>?, _ fieldIndex: Int32,
    ...
    // Only what a decompiler is for. Claiming every file would make the host ask this plugin for the
    // text of everything the search walks, and each miss would still cost a registry round trip.
    guard kind == "class" else { return -3 }   // PC_FT_FIELDEMPTY
    ...
    guard PluginDecompilerOptions.read(configRoot: configRoot).allowSearchDecompile else { return -3 }
    guard let source = decompiledSource(path: path, kind: kind, configRoot: configRoot) else {
        return -3
    }
...
private func decompiledSource(path: String, kind: String, configRoot: String) -> String? {
