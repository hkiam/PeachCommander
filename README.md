...
- Walk into filesystem images the same way — a router's SquashFS rootfs, an ext4 partition, an NTFS volume, a whole partitioned disk image. Firmware with no partition table is searched for the filesystems buried in it, and the bootloader and kernel around them can be copied out
...
| **Filesystem Images** | Walk into a firmware or disk image — SquashFS, ext, Btrfs, JFFS2, UBIFS, cramfs, initramfs, FAT, exFAT, NTFS, MBR/GPT partitions. Carves filesystems out of firmware that has no partition table, and writes a layout report. Read-only, off by default |
...
- **Remote locations** (FTP/SFTP) aren't watched for outside changes, because those protocols offer no way to be told — **F2** / **Ctrl+R** re-reads them. Folders on this Mac and an archive you have open both update by themselves.
