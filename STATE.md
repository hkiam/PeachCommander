...
measurement could settle, and in two cases the measurement is what changed the fix.

**The terminal's ✕ was beside the tab, not in it.** It was a second `NSButton` in a stack with the
tab's own — two bezels one point apart, with the *next tab* immediately to its right, which is what
it looked like it would close. It is a subview of the tab button now (`TerminalTabButton` reserves
trailing room for it in `intrinsicContentSize`, or the ✕ sits over the last letter of a long folder
name). Two things a screenshot caught and nothing else would have: the `xmark` symbol at its natural
weight comes out taller than the 11 pt name beside it and reads as the tab's main control, and a ✕
tinted with the theme's secondary text is dark-on-blue in the selected tab — the one a user is most
likely to close. 9 pt, and `alternateSelectedControlTextColor` when the tab is on.

**Closing a tab asks now whether or not something is running in it.** It used to ask only for a
running job, on the grounds that an idle shell is worth nothing; but the tab is also where everything
the shell has printed lives. A tab with a job still names the job, because that is the answer that
...
wrong line: what a preset is for is "show me this pair of folders the way I always look at them", and
half of that is which rows are on screen, so every load ended with the same two controls being set by
hand again. They round-trip now and are applied to the rows already on screen — the opposite of what
the advanced filter does one line above, and right for the same reason: this one re-reads a
comparison that has already happened, so there is nothing stale to warn about.

**The window also opens on the preset last used, and ships one to start from.** "Last used" is a date
*on the preset* rather than a file beside the list — a second file is a second thing that can
disagree with the first. It is stamped in `presetSelected` and not in `apply`, or opening the window
would re-stamp whatever it opened on and "last used" would come to mean "last opened". The shipped
**Default** holds the window's own defaults and nothing cleverer: a default that also changed what a
comparison *does* would make the first run of this window differ from every run of it before, which
is a different promise. It is seeded from the *absence of the file* and not from an empty list, so
deleting it makes it stay gone — a deletable item that comes back is the one failure it may not have,
and it would read as the delete button not working.

**Escape closes that window**, which had no keyboard way out at all. While a scan or a run is in
flight it is the Stop button instead; the second press closes. `windowWillClose` cancels both anyway,
so this is not about losing work — it is about what happens when somebody presses Escape to get out
of a dialog and closes the window mid-copy instead.

**Full VM run: 172 scenarios, 171 green, and the one failure was my own assertion.** `find-searchin`
asserted `NSTextField=379x57` from a `windowlayout` dump. 379 is the field's *width* on a German
machine, where the label column measures 103 pt; the guest runs English at 94, so the field is 300
wide there. The height — the entire subject of the change — was 57 in both. Widths in that dialog
follow the label column, which is measured from the longest label *per language*, so no width in it
can be asserted at all. Replaced by `searchInRow=` in the field report, which is the row's height and
so also catches the case the field's own measurement cannot: a control given more height inside a row
that kept its own. Confirmed on the guest at label column 94.

New in the suite: `sync-presets` (seeds, saves, closes with Escape, reopens from the file on disk,
and asserts `visibleRows=1/3` — a count neither control alone can produce) and `find-searchin`.
`terminal-close-ask` grew the idle half, where an aborted modal is neither button and the tab must
still be there afterwards. All three at zero conflicts, pinned in the baseline by hand — never with
`--update-baseline`, which rewrites only the scenarios that ran.

Also: rebuilding the shipped Help Book turned up pages stale against their own sources in every
language (`git.html`, `copying-files.html` and six others) — unrelated to this work, but CI requires
the rebuild to leave no diff, so it is in this commit.
...
shows no such file — and reading it asks the engine for the log. A file rather than a window because
a file is what makes the *viewer's* search, its jump to a line, its encoding choice and its follow
work, and none of those belong to a window a plugin draws itself. The Show Logs item stays for the
quick look that does not move the panel.

Three decisions worth keeping:

  * **A real entry of that name wins.** A container that genuinely ships `/docker-logs.txt` shows its
    own file. Hiding a file that is really in the image is the worse trade, and a test holds it.
  * **Nothing may be written to it**, or a copy onto the row would create the real file that then
    shadows the virtual one — which reads exactly like the log having been overwritten. Checked on
    the destination rather than through `writeTarget`, which is asked about the *parent* and would
    not see the name being written.
  * **The row's timestamp is the moment it was listed.** `MemberStage` keys its temporary copies by
    size and modification time, so a log that reported neither changing would be fetched once and
    shown for the rest of the session.

One thing the fixture had to get right: the engine frames a non-TTY container's log the way it frames
an attached exec, so it is demultiplexed. The fixture serves it framed for that reason — a fixture
answering in plain text would let a reader that forgot the framing pass, and the symptom in the app
would be eight bytes of header in front of the first line.

**And one mistake of my own, worth writing down.** The scenario's first run opened a *local folder* in
the viewer. The cause was an anchored string replacement in `regress.py` that silently matched
nothing: `PLUGINS_ON` had gained an entry since I wrote the anchor, so `docker-log` was never added
and the plugin was simply off. `pfxmount` then found no volume, the panel stayed where it was, and
F3 viewed whatever the cursor was on. A scripted edit that does nothing looks exactly like a feature
that does not work.

## 2026-09-14 — the settings page and the lifecycle (F-497), and a second layout defect found by measuring

The provider's settings are a contributed page now (`container: settings`), beside the host's own.
Everything but the engine and the exec fallback had been reachable only by hand-editing
`docker.ini` — the budgets, the helper image, whether digest-named volumes are listed — which is a
reasonable thing for a file to be and an unreasonable thing for a feature to be. The file is still
written and still read; the page is the other way in.

**Start/Stop/Restart/Pause/Unpause** in the same context submenu, each asking first because they
change the machine rather than reading it. The argument for them got stronger while the plugin was
being built: deleting and renaming need a running container, so until now the provider's own refusal
sent the user to a terminal. A 304 from the engine — "it is already like that" — is success, and
reading it as a failure would report "the engine refused" for the one case where nothing was wrong.
The confirmation answers itself under `PC_DOCKER_CONFIRM`, because a modal a plugin raises stops an
automation run dead.

**And the page shipped with a layout defect that measuring found and nothing else could.** With its
rows pinned only at the top, nothing stopped the view from being nothing tall: the host — which sizes
a contributed page by what it says it needs — was handed **702x0**, while every control inside it
still had a sensible frame. Zero Auto Layout conflicts, and a screenshot of a settings window with an
empty page reads as a page that has not loaded yet. `bottomAnchor >= rows.bottomAnchor` gives it a
height; 514x260 after.

Getting it under the gate needed a third measuring channel. `windowlayout` stops two levels under the
content view and the accessibility dump stops as shallow, while a contributed page sits deeper, inside
the Settings window's own scroll view — both came back with the page nowhere in them. So the page
reports its own frames and values under `PC_DOCKER_SETTINGS_DUMP`, which says three things at once:
it was built, it was laid out, and it was filled from the file. One trap in writing that: the first
version dumped one runloop turn after `viewDidMoveToWindow` and caught the view at 514x0 — the host
had not sized it yet. Measuring too early is its own wrong answer, and it looks exactly like the
defect that was really there.

## 2026-09-14 — the container actions (F-495, F-496), and two host gaps older than the plugin

Inspect, Show Logs, Show Mounts, Copy ID, Jump to Volume and Open Compose Project, contributed
through `contrib.h` and gated on `panelScheme`. The plugin side is the small half. Building it found
two things in the *host* that were wrong before this plugin existed, and both were silent.

**`panelScheme` was documented and set nowhere.** `contrib.h` has named it since the ABI was written
— "the same keys the declarative `when` sees, e.g. … `panelScheme`" — and no Swift file ever set it.
So a plugin could not tell its own drive from the local disk, and the only gate available was the
cursor's *path*, which a folder of the user's own carries just as well: a Docker item would have
offered itself over a directory called "Compose Projects" in somebody's home folder.

**The panel's context menu had a second, smaller context.** `PanelListView.contributionContext()`
built five cursor keys and returned; it never reached `contribAugmentContext`, so `dir`, `targetDir`,
`activeSide`, `diskMapActive` and `configRoot` were all invisible to a **context-menu** `when` while
the identical expression worked in a menu and on a keybinding. One documented context, two
implementations, and an item gated on such a key simply never appeared. It now starts from the host's
context and overwrites the cursor keys, which is the only reason it had its own copy: a right-click
moves the cursor to the row under the pointer.

**And `openPath` could not reach into a mount.** Both entry points asked
`FileManager.fileExists` — the local disk and nothing else — so every path inside a mount failed the
check and the call returned in silence. No plugin could navigate its own drive, which is where its
paths come from. The question goes to the filesystem the panel is on now; a stat that fails there
answers "directory", so the listing fails where the user can see it rather than nothing happening.

**How they are proved.** `MainWindowController` and `PanelListView` have no unit tests — the app
target has no test bundle — so the VM scenario `docker-jump` is the test. It dumps the panel's
context menu **twice in one run with the plugin switched on both times**, on a local folder and
inside the mount, so the only thing that differs between the two dumps is the filesystem the panel is
showing; then it invokes Jump to Volume and asserts the panel *landed on the volume*, not that the
call was made. The first run of it failed on the menu half and that failure was the context-menu gap
— found by the scenario rather than by reading.

The plugin half has unit tests through `PcRunCommand` with a stub host table: the full id on the
clipboard, the jump's destination, the two refusals. One trap there: the tests must hold the
`PFXFileSystem` alive across the call. The plugin registers the mounted engine on connect and forgets
it on disconnect, so a discarded mount leaves a command with no engine — and every one of those tests
failed as "the command did nothing", which is also what a genuinely broken command looks like.
...
**The VM gate, and the defect it found.** Three scenarios — `docker-drive`, `docker-volume`,
`docker-connect` — with `Fixtures/dockerd.py` seeded into the guest on a Unix socket and
`~/pc-cfg/Docker/docker.ini` pointing at it, so the mount happens through the drive *chip*
(`pfxmount Docker` → `PfxConnectVolume`) rather than through a dialog: that is the path a click
takes, and it is the one that can silently publish nothing. Two things had to be learned to get
there. The harness passes a scenario's settings as **launch arguments**, not environment — `open`
hands over arguments and never the caller's environment — so a plugin that reads only
`ProcessInfo.environment` never sees them; `dockerProbe` now reads both, and the unit tests (which
`setenv` in-process) and the VM both work. And a modal opened from a script never returns, so the
connect dialog is shown without its modal loop under `PC_DOCKER_DIALOG_NOMODAL`, which is the only
way that window can be measured at all.

Measuring it was worth it: **the dialog's layout was wrong with zero Auto Layout conflicts.** A
programmatically created `NSTextField` hugs its content at 250, the same as the combo box beside it,
so AppKit split the row between them — 448 pt for the word "Engine:" and 408 for the field, in a
900 pt window. Satisfiable and merely wrong, which is exactly the class `CONVENTIONS.md` warns the
conflict count cannot see. 49 pt and 807 pt after the fix, and the scenario asserts both the new
width and the old one's signature as a negation.
...
## 2026-09-14 — the three follow-ups, and the one that was already built

The items left open after the review, closed before the merge. Two of the three turned out to be
smaller than they looked because the answer already existed in the repository.

**Comparing across an archive or a server side.** It needed two *local* folders, so a row inside a
zip beeped — which after the viewer work was the odd one out: looking at one side had learned to
unpack, comparing had not. Both sides go through the same `localCopy` now. Three things had to come
with it rather than after it: the columns are named after the **sides** (F-416's own facility, which
exists for exactly this — otherwise it reads "both.txt ↔ both.txt" with no way to tell which came
out of the archive), the staged side is handed over as **not editable**, because merging into a copy
and saving it would report success while the archive kept what it had, and each side is fetched
exactly once — the first draft called `localCopy` a second time just to get a failure message, which
would have meant extracting a member twice to say why it could not be extracted.

**The temp copies.** I had written my own directory and was going to write my own pruning rule.
`ArchiveTempSweeper` already sweeps `PCStage-<pid>-…` at every launch and decides by the **owner
process**, not by age — which is strictly better than what I would have written: a copy is never
taken out from under a viewer that still has it open, and a crash does not leave one sitting for a
day. Naming the root that way was the whole change. The copies are `0o444` now as well, like
`MemberStage`'s, so "read-only temporary copy" is a fact rather than a sentence in a comment.

**The rest of the hex window's summary.** All four of its sentences used `NSLocalizedString`, which
the extraction does not see, so none of them had ever been in the catalogue. The offset one cannot go
through `String(localized:)` unchanged — a `LocalizationValue` has no hexadecimal form and the number
is an address — so it is formatted first and interpolated as text. Four keys, eighteen languages.

**One VM run was spent on a scenario bug of mine**, and it is worth writing down: `syncdemo` mapped a
`.zip` path to an archive side and `syncopen` did not, so the new scenario opened a *directory* side
pointed at a file. Empty listing, every row one-sided, `canCompare=false` — a report that reads
exactly like the feature refusing to work. The mapping is one function both verbs use now.

Verified: `sync-compare-zip` (a local folder against an archive, with the one-sided row as the
control that must still be refused), plus `sync-view` and `diff-equal` re-run, all at zero conflicts;
`Tools/test.sh` green; the string, inventory, docs and scenario gates green. The full 155-scenario
run that preceded these commits is the one in `docs/generated/layout-regression`; these three
changes were checked against their own scenarios rather than by repeating it.
...
target. That one was merely a wrong error before; with a replace dialog in front of it, it would have
offered to delete the file the user was renaming. It is an `lstat` on device and inode now, asked only
on the local disk — out on a mount the path names a place that exists nowhere on this Mac.

**The part worth writing down is the mount.** `isInArchive` is defined as `!(fs is LocalFS)`, so every
server and plugin mount went down the archive branch of `showRenameFile`, found no zip path and
returned in silence: Shift+F6 did nothing at all out there, and `PfxRenMov` had no route from the
keyboard. The same confusion had already been fixed once for delete. I fixed the gate, reasoned from
the types that it was now correct, and **the verification run said it still did nothing** — a second
gate of exactly the same kind sat behind it, in `cursorItemName()`, which carries its own
`guard fs is LocalFS`. Rightly so: it exists for link creation and a symlink has to be made on a disk.
So the rename asks a question about the name now, not about the volume. Reading the code found the
first gate and would not have found the second; only running it did.

Measured, and then measured again with the defect put back:

```
cursor=accountsd (640)     answersleft=false   ← fixed:  the dialog opened on the mount
cursor=accountsd (640)     answersleft=true    ← defect: nothing asked
```

`answersleft` is the whole instrument here — a queued answer that comes back **unused** is how a
silently swallowed command is caught, since there is nothing on screen and nothing in the log either
way.

**A local control proves nothing for this, and that cost a run.** On a local panel in details view
`cm_RenameOnly` takes the in-cell path, builds no `InputDialog`, and consumes no answer — which reads
exactly like the failure it is supposed to rule out. The `rename-replace` scenario therefore puts the
panel in brief view first, and says so where the next person will read it.

Both are scenarios now: `rename-replace` (cancel and replace in one run, because the confirmation is a
queue and one slot would answer both the same way) and `rename-mount`. The confirmation needed an
automation hook of its own — `InputDialog` has had one since the harness existed, an `NSAlert` has
none, and a modal one does not go unanswered but hangs the run inside its nested runloop. `replaceanswer`
and `replaceanswersleft` are that, and the second half matters: a build that replaces **without asking**
leaves exactly the same files behind as one that asked and was told to go ahead.

**An end-to-end mount rename could not be tested, and the reason is worth knowing before someone
tries again.** The obvious fixture is S3: the plugin has `PfxRenMov` and the guest already runs a
fixture server. But that server is deliberately anonymous — `_verify_signature` answers an unsigned
write with `403 AccessDenied`, because what it models is a *public bucket*, which is what the existing
`s3-mount` scenario is about. A signed profile needs the secret, and the secret is in the Keychain;
the plugin's environment route (`PC_S3_ENDPOINT`, `PC_S3_SECRET_KEY`) cannot be reached either,
because `regress-guest.sh` can deliver only `-KEY value` **arguments** and `S3Environment` reads
`ProcessInfo.environment`. SFTP is no better: no scenario mounts a *panel* on a server at all — the
...
curly ones — so a new curly one would have
been a twin differing in a glyph. The outlier moved to the house style instead, the two became one key
already translated into eighteen languages, and the new button reuses `Overwrite`, which is the word
this app already uses for the same act. Five genuinely new strings, in all nineteen.
