...
| Parity inventory | Re-audited 2026-08-04 against evidence: **147 done · 23 partial · 2 todo · 7 n/a-macos · 2 post-1.0** (181 rows). **P1 alone: 108 done · 4 partial · 1 todo.** The previous line claimed 59/70/43; of the 18 `todo` rows, 16 were implemented (one of them multimedia playback, complete with an `AVPlayerView`), and of the 50 P1 `partial` rows, 46 were complete — most "missing" sub-parts were only missing from my first grep. **The remaining core-parity work is all in FTP/FTPS**: F-193 (a whole .zip may be one side of a sync, an FTP site may not), F-212 (REST-based resume), F-213 (implicit FTPS only; explicit AUTH TLS not negotiated). Plus, by decision or blocked: F-237 (SFTP built in rather than a PFX plugin), F-310 signing, F-312 Sparkle. Every audited row carries an `ev:` pointer that `Tools/check-inventory.py` must resolve — it rejected six of my own pointers on the first run; 89 older `done` rows still carry none. |
