...
| Parity inventory | Re-audited 2026-08-04: **101 done · 68 partial · 3 todo · 7 n/a-macos · 2 post-1.0** (181 rows). The previous line claimed 43 todo, but of the 18 rows still marked so, **16 were implemented** — one of them multimedia playback, complete with an `AVPlayerView`. Genuinely open: F-193 (sync with an archive or an FTP site as one side — the engine works on `FileManager` directly), F-216 (FXP, P3), F-312 (Sparkle, blocked on Apple credentials). Rows touched now carry an `ev:` pointer that `Tools/check-inventory.py` must be able to resolve; 89 older `done` rows still have none. See docs/product/feature-inventory.md. |
