...
### One red scenario, and it is not this work's

The full VM run after all of the above is **113 of 114**. The one failure is `plugin-context-menu`,
which asserts the AI plugin's context items and reports them missing.

It is *not* caused by anything here, and the evidence is a control rather than an argument: the
scenario passes when run alone (`--only plugin-context-menu`, exit 0) and fails with **any**
predecessor — `--only main-window,plugin-context-menu` fails exactly the same way, with no macro
scenario anywhere in the run. The changes to the harness in this work are additive and do not touch
the `plugins.ini` branch the scenario depends on, and `~/pc-cfg/plugins.ini` was verified correct in
the guest at the moment of the failure (`Enabled=AI Assistant`).

So: the plugin is switched on in the config and contributes nothing, in every run where it is not the
first thing that happens. Recorded rather than written off, because "passes with `--only`, fails in a
full run" is precisely the shape that cost this project six scenarios and several months once already.
The leading hypothesis is timing — the AI plugin reaches for Apple Intelligence at startup (a
`#SAEStatus` line sits in the scenario's log) and the context menu is dumped before its contributions
arrive, with the first scenario of a run getting different warm-up than every later one. Untested.
Reproducer: `Tools/vm/regress.py --only main-window,plugin-context-menu`.
