...
- Comments with **line breaks** and comments in **UTF-16** are read and written the way Total Commander does: a line break is stored as `\n` followed by the two marker bytes TC registered for it, and a file that was UTF-16 stays UTF-16 when you edit one comment in it. Without that marker a `\n` in somebody's comment is two characters they typed, and it is left alone.
