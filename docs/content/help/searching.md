...
3. Turn on **Search inside archives** to look inside the archives the search finds — the same formats you can open with Enter, including any a packer plugin adds. Anything the search could not look into is reported when it finishes — an archive it could not open, one that needs a password, one too large or too deeply nested, and a **folder it could not read**. The count is next to the number of hits, and the list names each one with the reason. This matters more than it sounds: a search that quietly walks past something reads exactly like "what you are looking for is not there".
