...
- Natural (numeric) sorting is on by default, so `file2` comes before `file10` instead of after it. You can switch it off in Configuration > Options under the display settings.
