...
# Common ABI (pc_common)

> Source: `Plugins/SDK/pc_common.h` — this page is generated from that header by `docs/scripts/gen-api-reference.py`; edit the header, not this page.
...
|---|---|---|
| `PC_API_VERSION` | `1` |  |
| `PC_API_MIN_SUPPORTED` | `1` |  |
| `PC_CONTINUE` | `1` |  |
| `PC_ABORT` | `0` |  |
| `PC_SKIP` | `0` | skip this file (advance the read position only) |
| `PC_TEST` | `1` | test the file (read + verify, no output written) |
| `PC_EXTRACT` | `2` | extract the file to destPath/destName |
| `PC_OK` | `0` |  |
| `PC_E_END_ARCHIVE` | `10` | no more headers (ReadHeader reached the end) |
| `PC_E_NO_MEMORY` | `11` |  |
| `PC_E_BAD_DATA` | `12` | CRC / structural error |
| `PC_E_BAD_ARCHIVE` | `13` |  |
| `PC_E_UNKNOWN_FMT` | `14` |  |
| `PC_E_EOPEN` | `15` | cannot open the source/archive file |
| `PC_E_ECREATE` | `16` | cannot create the target file |
| `PC_E_ECLOSE` | `17` |  |
| `PC_E_EREAD` | `18` |  |
| `PC_E_EWRITE` | `19` |  |
| `PC_E_SMALL_BUF` | `20` |  |
| `PC_E_EABORTED` | `21` | user aborted via the progress callback |
| `PC_E_NO_FILES` | `22` |  |
| `PC_E_TOO_MANY` | `23` |  |
...
| `PC_CAP_DELETE` | `0x0008` | can delete files from an archive |
| `PC_CAP_OPTIONS` | `0x0010` | has a configuration dialog |
| `PC_CAP_MEMPACK` | `0x0020` | supports the StartMemPack API |
| `PC_CAP_BY_CONTENT` | `0x0040` | CanYouHandleThisFile detects by content |
| `PC_CAP_SEARCHTEXT` | `0x0080` | archive contents are searchable as text |
| `PC_CAP_HIDE` | `0x0100` | do not show as a separate packer |
| `PC_CAP_ENCRYPT` | `0x0200` | supports encryption |
| `PC_CAP_MULTITHREAD` | `0x0400` | host need not serialise calls |
| `PC_CAP_RANDOM_ACCESS` | `0x0800` |  |
| `PC_VOL_ASK` | `0` | ask the user for the next volume |
| `PC_VOL_NOTIFY` | `1` | just notify that a volume changed |
| `PC_CRYPT_SAVE_PASSWORD` | `1` |  |
| `PC_CRYPT_LOAD_PASSWORD` | `2` |  |
| `PC_CRYPT_COPY_PASSWORD` | `3` | load without prompting |
| `PC_CRYPT_DELETE` | `4` |  |
