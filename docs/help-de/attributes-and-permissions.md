...
- Kommentare werden in einer Datei `descript.ion` neben Ihren Elementen gespeichert und können je nach Ihren Einstellungen auch als Finder-Kommentare geführt werden. Beide werden beim Anzeigen eines Kommentars gelesen. Das Format ist dasselbe, das Total Commander und mehrere andere Dateimanager verwenden, ein hier geschriebener Kommentar ist dort also lesbar.
- Kommentare mit **Zeilenumbrüchen** und Kommentare in **UTF-16** werden so gelesen und geschrieben, wie Total Commander es tut: ein Zeilenumbruch steht als `\n` gefolgt von den zwei Markierungsbytes, die TC dafür registriert hat, und eine Datei, die UTF-16 war, bleibt UTF-16, wenn Sie einen Kommentar darin ändern. Ohne diese Markierung sind `\n` in einem Kommentar zwei getippte Zeichen und bleiben unangetastet.
- **Ein Kommentar folgt der Datei.** Kopieren, Verschieben oder Umbenennen nimmt ihn mit — beim Verschieben und Kopieren in die `descript.ion` des Zielordners, beim Umbenennen auf den neuen Namen, auch wenn Sie das Umbenennen widerrufen. Ausnahme ist das Anhängen einer Datei an eine andere: die bleibende Datei behält ihren eigenen Kommentar, weil sie dieselbe Datei bleibt.
- Ist das Notizen-Plugin eingeschaltet, zeigt und bearbeitet seine Seitenleiste denselben Kommentar über dem Notiztext, damit es nicht zwei Orte für dasselbe gibt.
