...
- **Markdown and HTML** — pulse F3 sobre un archivo `.md` o `.html` y léalo con formato en lugar de como código fuente, con los diagramas ` ```mermaid ` dibujados y las matemáticas `$…$` compuestas en su Mac. No se descarga nada y ninguna parte del documento se envía a ningún sitio.
