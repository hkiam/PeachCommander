...
- Les commentaires avec des **sauts de ligne** et ceux en **UTF-16** sont lus et écrits comme le fait Total Commander : un saut de ligne est stocké sous la forme `\n` suivi des deux octets de marquage que TC a fait enregistrer, et un fichier qui était en UTF-16 le reste quand vous modifiez un commentaire dedans. Sans ce marquage, un `\n` dans le commentaire de quelqu'un est deux caractères qu'il a tapés, et on n'y touche pas.
