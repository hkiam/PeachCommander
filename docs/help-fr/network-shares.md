...
3. Dans le dialogue **Se connecter au serveur**, saisissez l'adresse du serveur. Vous pouvez indiquer :
   ...
   - un chemin de style Windows, par exemple `\\fileserver\projects\reports`
...
- Un chemin à la mode Windows fonctionne aussi dans **Aller au dossier** et dans la barre de chemin au-dessus d'un panneau, pas seulement dans « Se connecter au serveur ». Tapez-y `\\fileserver\projects\reports` et vous arrivez dans ce dossier.
- Si le partage est déjà connecté, vous allez directement au dossier — sans feuille d'authentification et sans second aller-retour vers le serveur. Seul le partage lui-même est monté ; les dossiers en dessous sont atteints par une navigation ordinaire, de sorte que tout l'arbre au-dessus reste accessible.
