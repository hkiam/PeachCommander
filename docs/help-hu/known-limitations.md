...
- **A felosztott (többrészes) ZIP-archívumok megnyithatók, de minden résznek meg kell lennie.** A szokásos ZIP — beleértve a ZIP64-et, tehát a 65 535 elemnél többet vagy a 4 GB-nál nagyobbat — valamint a TAR és a gzippel tömörített TAR közvetlenül mappaként nyílik. A több fájlra osztott archívum is megnyílik: nyomjon Entert a `.z01`, `.z02`, … készlet `.zip` fájlján, vagy a `name.zip.001` készlet `.001` fájlján. Minden résznek ugyanabban a mappában kell lennie, és az a készlet, amelyből egy hiányzik, elutasításra kerül ahelyett, hogy félig beolvasva nyílna meg. A felosztott TAR-archívumokra ez nem vonatkozik.
...
- **A távoli helyeket nem figyeli a program; a megnyitott archívumot mostantól igen.** Egy mappa ezen a Macen magától frissül, amint egy másik program fájlt hoz létre, módosít vagy töröl benne — és így tesz az archívum is, amelybe belenézett: a `.zip` helyi fájl, így ha valami felülírja, a panel újraolvassa. A távoli helyeket (FTP vagy SFTP) nem figyeli, mert azok a protokollok nem kínálnak módot az értesítésre — nyomjon F2-t vagy Ctrl+R-t.

## Egyéb jelenlegi határok

- **A nagyon hosszú útvonalak működnek, a Kuka kivételével.** A macOS hívási argumentumként elutasít minden 1024 bájtnál hosszabb útvonalat, és ilyen mélyen beágyazott mappák előfordulnak. A böngészés, megnyitás, másolás, áthelyezés, átnevezés, létrehozás és végleges törlés mind eléri őket. Az egyetlen kivétel a **Kukába helyezés**: a macOS nem kínál módot olyan fájl kidobására, amelyet nem tud megnevezni, ezért a Delete ott hibát jelez — a Shift+Delete (végleges törlés) működik.
