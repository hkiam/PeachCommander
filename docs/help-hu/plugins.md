...
- **Markdown and HTML** — nyomja meg az F3-at egy `.md` vagy `.html` fájlon, és olvassa formázva forrás helyett, kirajzolt ` ```mermaid ` diagramokkal és az Ön Macén szedett `$…$` matematikával. Semmi sem töltődik le, és a dokumentum egyetlen része sem kerül elküldésre.
