...
1. 설정 ▸ 플러그인…을 선택합니다.
2. **폴더에서 설치…**를 클릭합니다.
3. 플러그인 번들 또는 그것을 담은 `.zip`을 고르고 확인합니다. 플러그인이 목록에 추가되고 켜집니다.

## 플러그인 제거

1. 플러그인 창에서 목록의 플러그인을 선택합니다.
2. **제거**를 클릭합니다. 기본 기능은 영향을 받지 않으며, 선택한 플러그인만 제거됩니다.

## 참고

- 플러그인 목록은 이름과 위치와 함께 각 플러그인의 유형과 인터페이스 버전을 보여 주므로 무엇이 설치되어 있는지 확인할 수 있습니다.
- 설치된 플러그인이 없으면 창은 **폴더에서 설치…**로 안내하는 짧은 안내를 표시합니다.
- 일부 플러그인은 켜져 있을 때만 자신의 열, 메뉴 항목, 패널 위치를 더합니다. 기대한 기능이 보이지 않으면 여기서 해당 플러그인이 켜져 있는지 확인하세요.
