...
- Komentarze z **podziałami wiersza** i komentarze w **UTF-16** są czytane i zapisywane tak, jak robi to Total Commander: podział wiersza jest przechowywany jako `\n`, po którym następują dwa bajty znacznika, jaki TC zarejestrował do tego celu, a plik, który był w UTF-16, pozostaje w UTF-16, gdy zmienisz w nim jeden komentarz. Bez tego znacznika `\n` w czyimś komentarzu to dwa wpisane znaki i pozostają nietknięte.
