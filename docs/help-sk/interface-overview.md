...
- **Lišta jednotiek**: jedno tlačidlo na každý dostupný disk alebo zväzok, pri každom voľné miesto. Kliknutím na zväzok doň prepnete tento panel; kliknutím pravým tlačidlom ho vysuniete — ponúka sa pri vymeniteľných zväzkoch a pripojených obrazoch diskov, zošedivené pri spúšťacom disku a sieťových zdieľaniach. Zásuvné moduly môžu pridať vlastné jednotky — Task Manager je jednou z nich — a správajú sa ako každý iný zväzok: panel sa do nej prepne, tlačidlo zostane vybrané a karta nesie názov jednotky. Každé tlačidlo nesie vlastnú ikonu zväzku — tú istú, ktorú ukazuje Finder — takže pevný disk, USB kľúč, pripojený obraz disku a sieťové zdieľanie rozoznáte na prvý pohľad.
