...
- Перерване завантаження продовжується з того місця, де зупинилося: якщо файл частково вже є і сервер дозволяє перезапуск, передається лише те, чого не вистачає. Сервер, який відмовить, просто почне файл заново. Надсилання відновлюється так само, якщо файл на сервері коротший за надсилений.
