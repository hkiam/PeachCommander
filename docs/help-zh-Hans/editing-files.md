...
4. 按 Cmd+S（或单击“保存”）写入你的更改。保存会替换文件；若希望把先前的内容留在它旁边，请在设置 ▸ 编辑/查看中打开备份。
...
- 若要使用**正则表达式**，请选择 搜索 ▸ *以正则表达式查找…*（Ctrl+Cmd+F）或 *以正则表达式替换…*（Ctrl+Opt+Cmd+F）。`^` 和 `$` 匹配行首和行尾，替换文本中的 `$1` 表示第一个捕获组——因此把 `(\w+) (\d+)` 替换为 `$2=$1`，`alpha 11` 就变成 `11=alpha`。**仅在所选内容中**可将更改限制在选中的文本内；**全部替换**将所有匹配一次改写完，可用 Cmd+Z 一步撤销。
- 查找下一个（Cmd+G）沿用你最后使用的那种搜索，无论是普通搜索还是模式。无法编译的模式会在对话框中报告，而不是悄悄地什么也找不到。
...
4. 按 Cmd+S 保存。与文本编辑器一样，只有在你打开备份后才会保留先前的内容。

## 你正在编辑的文件中的字符串

十六进制编辑器有着与查看器相同的**字符串**面板：文件中每一段可读的文本，四种编码同时呈现，点击一下就把光标和选区放到它上面。

- 它读取的是你编辑之后的字节，而不是磁盘上的字节，因此在一次插入把下方内容整体移位之后，偏移仍然指向正确的位置。
- 列表会跟随你的编辑：改动一个字节，停下输入片刻后它便重新建立。
- 完整说明见[查看文件](viewing-files.md#read-the-strings-in-a-binary)，在这里的行为完全相同。
...
- 除非你要求，两个编辑器都不会保留备份。在设置 ▸ 编辑/查看中打开“保存时保留先前内容的备份副本（.bak）”，首次保存便会把原始文件以 `name.bak` 写在文件旁边，意外的更改因此容易撤销。
