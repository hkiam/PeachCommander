...
1. 选择 配置 ▸ 插件…。
2. 点按 **从文件夹安装…**。
3. 选择一个插件包或包含它的 `.zip`，然后确认。插件会被添加到列表并启用。

## 移除插件

1. 在插件窗口中，选择列表里的插件。
2. 点按 **移除**。内置功能不受影响；只移除所选插件。

## 备注

- 插件列表会在名称和位置旁显示每个插件的类型和接口版本，便于你确认已安装的内容。
- 如果没有安装任何插件，窗口会显示一段简短提示，指引你使用 **从文件夹安装…**。
- 某些插件只在启用时才添加自己的列、菜单项或面板位置。如果你期望的功能缺失，请检查其插件是否已在此处开启。
